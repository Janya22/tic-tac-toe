package ticTacToe;


import java.util.Date;
import java.util.HashMap;
import java.util.List;
import java.util.Random;
import java.util.Map;
/**
 * A policy iteration agent. You should implement the following methods:
 * (1) {@link PolicyIterationAgent#evaluatePolicy}: this is the policy evaluation step from your lectures
 * (2) {@link PolicyIterationAgent#improvePolicy}: this is the policy improvement step from your lectures
 * (3) {@link PolicyIterationAgent#train}: this is a method that should runs/alternate (1) and (2) until convergence. 
 * 
 * NOTE: there are two types of convergence involved in Policy Iteration: Convergence of the Values of the current policy, 
 * and Convergence of the current policy to the optimal policy.
 * The former happens when the values of the current policy no longer improve by much (i.e. the maximum improvement is less than 
 * some small delta). The latter happens when the policy improvement step no longer updates the policy, i.e. the current policy 
 * is already optimal. The algorithm should stop when this happens.
 * 
 * @author ae187
 *
 */
public class PolicyIterationAgent extends Agent {

	/**
	 * This map is used to store the values of states according to the current policy (policy evaluation). 
	 */
	HashMap<Game, Double> policyValues=new HashMap<Game, Double>();
	
	/**
	 * This stores the current policy as a map from {@link Game}s to {@link Move}. 
	 */
	HashMap<Game, Move> curPolicy=new HashMap<Game, Move>();
	
	double discount=0.9;
	
	/**
	 * The mdp model used, see {@link TTTMDP}
	 */
	TTTMDP mdp;
	
	/**
	 * loads the policy from file if one exists. Policies should be stored in .pol files directly under the project folder.
	 */
	public PolicyIterationAgent() {
		super();
		this.mdp=new TTTMDP();
		initValues();
		initRandomPolicy();
		train();
		
		
	}
	
	
	/**
	 * Use this constructor to initialise your agent with an existing policy
	 * @param p
	 */
	public PolicyIterationAgent(Policy p) {
		super(p);
		
	}

	/**
	 * Use this constructor to initialise a learning agent with default MDP paramters (rewards, transitions, etc) as specified in 
	 * {@link TTTMDP}
	 * @param discountFactor
	 */
	public PolicyIterationAgent(double discountFactor) {
		
		this.discount=discountFactor;
		this.mdp=new TTTMDP();
		initValues();
		initRandomPolicy();
		train();
	}
	/**
	 * Use this constructor to set the various parameters of the Tic-Tac-Toe MDP
	 * @param discountFactor
	 * @param winningReward
	 * @param losingReward
	 * @param livingReward
	 * @param drawReward
	 */
	public PolicyIterationAgent(double discountFactor, double winningReward, double losingReward, double livingReward, double drawReward)
	{
		this.discount=discountFactor;
		this.mdp=new TTTMDP(winningReward, losingReward, livingReward, drawReward);
		initValues();
		initRandomPolicy();
		train();
	}
	/**
	 * Initialises the {@link #policyValues} map, and sets the initial value of all states to 0 
	 * (V0 under some policy pi ({@link #curPolicy} from the lectures). Uses {@link Game#inverseHash} and {@link Game#generateAllValidGames(char)} to do this. 
	 * 
	 */
	public void initValues()
	{
		List<Game> allGames=Game.generateAllValidGames('X');//all valid games where it is X's turn, or it's terminal.
		for(Game g: allGames)
			this.policyValues.put(g, 0.0);
		
	}
	
	/**
	 *  You should implement this method to initially generate a random policy, i.e. fill the {@link #curPolicy} for every state. Take care that the moves you choose
	 *  for each state ARE VALID. You can use the {@link Game#getPossibleMoves()} method to get a list of valid moves and choose 
	 *  randomly between them. 
	 */
	public void initRandomPolicy() {
	    for (Game g : Game.generateAllValidGames('X')) {//loops through all valid game states
	        if (g.isTerminal()) {
	            continue; // if it is a terminal state you move to the next game state (no policy for terminal state)
	        }
	        List<Move> possibleMoves = g.getPossibleMoves();//get all the possible move for the game state
	        if (possibleMoves.isEmpty()) {
	            continue; // if no valid moves then go to next game state
	        }
	        Move rMove = possibleMoves.get(new Random().nextInt(possibleMoves.size()));//pick a random policy from the range of all possible moves
	        curPolicy.put(g, rMove);//updates curPolicy with current game state and randomMove generated
	    }

	    // Set the current policy to the agent's policy
	    this.policy = new Policy(curPolicy);
	}



	
	
	/**
	 * Performs policy evaluation steps until the maximum change in values is less than {@code delta}, in other words
	 * until the values under the currrent policy converge. After running this method, 
	 * the {@link PolicyIterationAgent#policyValues} map should contain the values of each reachable state under the current policy. 
	 * You should use the {@link TTTMDP} {@link PolicyIterationAgent#mdp} provided to do this.
	 *
	 * @param delta
	 */
	protected void evaluatePolicy(double delta) {
	    // Initialize convergence flag to keep track if the values under current policy converged
	    boolean valuesConverged;

	    // Repeats until the values converge
	    do {
	        valuesConverged = true; // Assume convergence unless proven otherwise
	        
	     // Temporary map to store updated values of each state
	        HashMap<Game, Double> updatedValues = new HashMap<>();

	        // Iterate over all states in the policy
	        for (Game g : Game.generateAllValidGames('X')) {
	            if (g.isTerminal()) {
	                updatedValues.put(g, 0.0); // if it is a Terminal set its value to 0, and move to the next game state 
	                continue;
	            }

	            Move gMove = curPolicy.get(g); // Move for the current game state that was stored in current policy
	            double value = 0.0;// for all game states initial value is assigned as 0

	            // Calculate the expected value for the current state
	            List<TransitionProb> transitions = mdp.generateTransitions(g, gMove);//get all transitions for the current game state and move
	            for (TransitionProb T : transitions) {
	                Game nextState = T.outcome.sPrime;//destination state from current state
	                double reward = T.outcome.localReward;//reward for the transition
	                double nextStateValue = policyValues.get(nextState); // Value of next state

	                // Bellman equation for expected value
	                value += T.prob * (reward + discount * nextStateValue);
	            }

	            updatedValues.put(g, value);//store value of each state 

	           
	            if (Math.abs(value - policyValues.get(g)) > delta) {
	                valuesConverged = false;// if the change in values is more than the threshold delta then values are not converged
	            }
	        }

	        // Update the policy values map
	        policyValues = (HashMap<Game, Double>)updatedValues;

	    } while (!valuesConverged); // Continue until convergence
	}

		
	
	
	/**This method should be run AFTER the {@link PolicyIterationAgent#evaluatePolicy} train method to improve the current policy according to 
	 * {@link PolicyIterationAgent#policyValues}. You will need to do a single step of expectimax from each game (state) key in {@link PolicyIterationAgent#curPolicy} 
	 * to look for a move/action that potentially improves the current policy. 
	 * 
	 * @return true if the policy improved. Returns false if there was no improvement, i.e. the policy already returned the optimal actions.
	 */
	protected boolean improvePolicy() {
	    boolean policyImproved = false; //Initialize policy improvement flag to check if the new policy is better than the old

	    for (Game g : Game.generateAllValidGames('X')) {//iterate over all valid game states
	        if (g.isTerminal()) {//if it is a terminal state skip and move to next game state since no moves will be done in terminal state
	            continue;
	        }

	        List<Move> possibleMoves = g.getPossibleMoves();//get the list of all possible moves from a game state
	        Move optMove = null;//initialize the optimal move from that state to null
	        double maxValue = Double.NEGATIVE_INFINITY;//initialize the value of the optimal move to a lowest possible negative value

	        for (Move move : possibleMoves) {//run a for loop for every possible move a state
	            List<TransitionProb> transitions = mdp.generateTransitions(g, move);//get the list of all transitions for this state and move
	            double expectedValue = 0.0;// Initialize the expected value of this move to 0

	            for (TransitionProb T : transitions) {//run a for loop for all the transitions possible for the current state-action pair
	                Game nextState = T.outcome.sPrime;
	                double reward = T.outcome.localReward;
	                double nextStateValue = policyValues.get(nextState);
	                //expectedValue of this transition according to bellman equation
	                expectedValue += T.prob * (reward + discount * nextStateValue);
	            }
	            
	            if (expectedValue > maxValue) {//if the expectedValue calculated for this transition in this state is greater than the values of 
	            								//transitions so far update maxValue to this transition's expected value
	                maxValue = expectedValue;
	                optMove = move;//update the optimal move to this move
	            }
	        }

	        if (!optMove.equals(curPolicy.get(g))) {//if the optimal move is not equal to move of the state under current policy
	            curPolicy.put(g, optMove);//update current policy to the optimal move and flag the policy as improved
	            policyImproved = true;
	        }
	    }

	    return policyImproved;//returns boolean according to whether the policy was improved or not
	}


	
	/**
	 * The (convergence) delta
	 */
	double delta=0.1;
	
	/**
	 * This method should perform policy evaluation and policy improvement steps until convergence (i.e. until the policy
	 * no longer changes), and so uses your 
	 * {@link PolicyIterationAgent#evaluatePolicy} and {@link PolicyIterationAgent#improvePolicy} methods.
	 */
	public void train() {
	    initRandomPolicy();  // Initialize a random policy

	    boolean policyConverged;
	    do {
	        evaluatePolicy(delta);   // Evaluate the current policy
	        //improvePolicy is called to see if current policy can be improved
	        policyConverged = !improvePolicy();  // policyConverged will be set to true only when improve policy returns false
	    } while (!policyConverged);  // Repeat until the policy no longer changes

	    // setting final policy with the current policy after training
	    this.policy = new Policy(curPolicy);  
	}


	
	public static void main(String[] args) throws IllegalMoveException
	{
		/**
		 * Test code to run the Policy Iteration Agent agains a Human Agent.
		 */
		PolicyIterationAgent pi=new PolicyIterationAgent();
		
		HumanAgent h=new HumanAgent();
		
		Game g=new Game(pi, h, h);
		
		g.playOut();
		
		
	}
	

}
